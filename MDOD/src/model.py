...
class U2Net(nn.Module):
    def __init__(self, cfg: dict, out_ch: int = 1):
        ...
        # self.DCA = DCA(n=1,
        #                features=[int(64), int(128), int(256), int(512), int(512)],
        #                strides=[8, 4, 2, 1, 0.5],
        #                patch=288 // 8,
        #                spatial_att=True,
        #                channel_att=True,
    ...
    def forward(self, x: torch.Tensor) -> Union[torch.Tensor, List[torch.Tensor]]:
        ...
        # if self.training:
        #     encode_outputs[0], encode_outputs[1], encode_outputs[2], encode_outputs[3], encode_outputs[4] = self.DCA(
        #         [encode_outputs[0], encode_outputs[1], encode_outputs[2], encode_outputs[3], encode_outputs[4]])
        x = encode_outputs.pop()
        decode_outputs = [x]
        for m in self.decode_modules:
            x2 = encode_outputs.pop()
...
if __name__ == '__main__':
    model = u2net_full()
    weights = torch.load("../inference/model_defect.pth", map_location='cpu')
    if "model" in weights:
        model.load_state_dict(weights["model"])
    else:
        model.load_state_dict(weights)
    convert_onnx(model, "u2net_full.onnx")
