...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, dice_loss, focal_loss, cls_weights, num_classes, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    if local_rank == 0:
        print('Start Train')
        pbar = tqdm(total=epoch_step,desc='Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3)
    model_train.train()
    ...
    if local_rank == 0:
        ...
        pbar = tqdm(total=epoch_step_val, desc='Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3)
